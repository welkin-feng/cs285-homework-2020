import numpy as np
from cs285.infrastructure.utils import add_noise, convert_listofrollouts, get_pathlength
...
class ReplayBuffer(object):
    ...
    def add_rollouts(self, paths, noised=False):
        """
        Args:
            paths (list[np.ndarray]): list of path
            noised (bool):
        """
        ...
        if self.obs is None:
            ...
            self.concatenated_rews = np.concatenate(
                [self.concatenated_rews, concatenated_rews])[-self.max_size:]
            # DONE keep only latest max_size around
    ...
    def sample_recent_data(self, batch_size=1, concat_rew=True):
        ...
        Args:
            batch_size (int):
            concat_rew (bool): if True, return concatenated rews (np.ndarray) of recent (s_t, a_t),
                else return unconcatenated rews (list[list]) of recent paths.
        Returns:
            observations:
            actions:
            rewards:
