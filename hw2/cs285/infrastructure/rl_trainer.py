...
class RL_Trainer(object):
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, all_logs):
        ...
        print_log("Collecting data for eval...\n")
        ...
        if self.log_metrics:
            ...
            # log one path's reward example
            if (itr + 1) % 5 == 0:
                cumsum_reward = np.cumsum(paths[0]["reward"])
                for i, r in enumerate(cumsum_reward):
                    self.logger.log_scalar(r, f"reward{itr}", i)
