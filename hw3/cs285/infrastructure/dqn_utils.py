...
def get_env_kwargs(env_name):
    if env_name in ['MsPacman-v0', 'PongNoFrameskip-v4']:
        ...
        kwargs['optimizer_spec'] = lander_optimizer(),
...
######################
# Exploration schedule and Optimizer
######################
...
class MemoryOptimizedReplayBuffer(object):
    ...
    def _encode_observation(self, idx):
        """Return `self.frame_history_len` batch frames

        Args:
            idx (int): end index

        Returns:
            obs (np.ndarray): size [H, W, frame_history_len * 3]
        """
